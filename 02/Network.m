...
-(void(^)(response_t))get
{
    ...
    return ^(response_t response){
        ...
        NSURL * URL = [NSURL URLWithString:[NSString stringWithFormat:@"%@?%@",self.url_,self.params_.dict2encoded()].check()];
...
-(void(^)(response_t))post
{
    ...
    return ^(response_t response){
        ...
        NSMutableURLRequest * mutableURLRequest = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:self.url_.check()]];
                      ...
                      NSString * body;
        if(self.urlencoded_){
          body = self.params_.dict2encoded();
...
-(void)sessionDataTask:(NSMutableURLRequest *) mutableURLRequest{
    ...
    NSURLSessionDataTask * dataTask = [session dataTaskWithRequest:mutableURLRequest completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        if (error == nil) {
            ...
            @catch(NSException *e){
                dispatch_async(dispatch_get_main_queue(), ^{
                    NSError *error = [NSError errorWithDomain:NSURLErrorDomain code:110 userInfo:@{NSLocalizedDescriptionKey:@"JSON反序列化错误"}];
...
-(void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didCompleteWithError:(NSError *)error
{
    ...
    if (self.file_) self.file_(self.localUrl,error);
...
- (void)URLSession:(NSURLSession *)session downloadTask:(NSURLSessionDownloadTask *)downloadTask didFinishDownloadingToURL:(NSURL *)location{
    ...
    self.localUrl = location.filePathURL ;
}
@end
